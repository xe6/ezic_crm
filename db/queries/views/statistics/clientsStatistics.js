...
const clientsStatiscs = `
    create view if not exists \`Clients_statistics\` as
        ...
        SELECT IFNULL(QUANTITY,0), IFNULL(STATUS, 'enrolled') as status, month, id as ord
        FROM Sum_enrolled_clients RIGHT JOIN Months ON Sum_enrolled_clients.month_name = Months.month

        UNION ALL

        SELECT IFNULL(QUANTITY,0), IFNULL(STATUS, 'gone') as status, month, id as ord
